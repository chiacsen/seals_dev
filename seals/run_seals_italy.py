...
if __name__ == '__main__':
    ...
    project_folder_name = current_csv_name.replace('.csv', '') + '_run'

    p.project_name = project_folder_name

    current_script_folder = os.path.dirname(os.path.abspath(__file__))
    main_repo_folder = os.path.dirname(current_script_folder)
    ...
    #p.scenario_definitions_filename = 'italy_ssp2_rcp45.csv'
    #p.scenario_definitions_path = os.path.join(p.input_dir, p.scenario_definitions_filename)
    #p.scenario_definitions_path = os.path.join(p.cur_dir, '..', 'scenarios', p.scenario_definitions_filename)
    p.scenario_definitions_filename = current_csv_name
    p.scenario_definitions_path = os.path.join(main_repo_folder, 'scenarios', current_csv_name)
    if not os.path.exists(p.scenario_definitions_path):
        raise FileNotFoundError(f"CRITICAL: Could not find {p.scenario_definitions_path}")
